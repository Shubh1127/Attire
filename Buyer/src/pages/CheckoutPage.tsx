...
const CheckoutPage: React.FC = () => {
  ...
  const { buyer, fetchCart, getProfile } = useBuyerContext();
    ...
    const checkAuthAndFetchProfile = async () => {
      try {
        ...
        if (token || supabaseUser) {
          ...
          // Fetch profile data only if it is empty
          if (!buyer || !buyer.addresses || buyer.addresses.length === 0) {
            await getProfile();
          }
        } else {
  ...
  // Set default address once profile is loaded
  useEffect(() => {
    if (buyer?.addresses?.length > 0) {
      const defaultAddress = buyer.addresses.find((addr) => addr.isDefault);
      setSelectedAddressId(defaultAddress?.id || buyer.addresses[0]?.id || null);
    }
  }, [buyer?.addresses]);

  // Fetch cart data if empty
  useEffect(() => {
    if (isAuthenticated && (!buyer?.cart || buyer.cart.length === 0)) {
      fetchCart();
    }
  }, [isAuthenticated, buyer?.cart, fetchCart]);

  // Redirect to cart if empty
  useEffect(() => {
    if (isAuthenticated && buyer?.cart?.length === 0) {
      navigate('/cart');
    }
  }, [isAuthenticated, buyer?.cart, navigate]);

  const selectedAddress = buyer?.addresses?.find((addr) => addr.id === selectedAddressId);
        ...
        <div>
          <div className={`rounded-lg shadow-sm border p-6 sticky top-24 ${
            ...
            <div className={`max-h-60 overflow-y-auto mb-4 ${
              theme === 'dark' ? 'scrollbar-thumb-navy-600' : ''
            }`}>
              {buyer?.cart?.map((item) => (
                <div key={item._id} className={`flex py-2 border-b ${
                  theme === 'dark' ? 'border-navy-700' : 'border-gray-100'
                } last:border-b-0`}>
            ...
            {selectedAddress ? (
              ...
              <Button
                ...
                size="lg"
                className={`mt-4 ${
                  theme === 'dark' ? 'bg-navy-700 text-green-400' : ''
                }`}
                disabled
              >
